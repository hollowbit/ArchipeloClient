			...
			if (entity.getAnimationManager().getAnimationMeta().equals("")) {//If there is no meta data
				...
				if (useAnimationMeta == null) {
					...
					if (useAnimationMeta.useItem.equipType == ItemType.EQUIP_INDEX_USABLE)
						animationLength = useAnimationMeta.useItem.useAnimationLengths[useAnimationMeta.useType % useAnimationMeta.useItem.numOfUseAnimations];
				} else {
					if (useAnimationMeta.useItem.equipType == ItemType.EQUIP_INDEX_USABLE)
						animationLength = useAnimationMeta.useItem.useAnimationLengths[useAnimationMeta.useType % useAnimationMeta.useItem.numOfUseAnimations];
				}
		...
		if (renderUsableFirst) {
			if (isUseAnimation && useAnimationMeta != null) {
				if (useAnimationMeta.useItem.equipType == ItemType.EQUIP_INDEX_USABLE) {//Only render use animation if item is usable
					...
					batch.draw(useAnimationMeta.useItem.getAnimationFrameForUsable(animationId, direction, stateTime, useAnimationMeta.style, useAnimationMeta.useType, useAnimationMeta.useItem.useAnimationLengths[useAnimationMeta.useType % useAnimationMeta.useItem.numOfUseAnimations]), x, y);
