...
public class ExtendedNet implements Net {
    ...
    /** @param host server address.
     * @param port server port.
     *  @param keystorePath Path to the self-signed keystore
     *  @param storeType Type of keystore (Usually "JKS")
     *  @param keyPass Password for keystore
     *  @param storePass Password for store
     * @return a new {@link WebSocket} instance, which allows to securely connect with the server. */
    public WebSocket newSecureWebSocket(final String host, final int port, final Files gdxFiles, final String keystorePath, final String storeType, final String keyPass, final String storePass) {
        return WebSockets.newSecureSocket(WebSockets.toSecureWebSocketUrl(host, port), gdxFiles, keystorePath, storeType, keyPass, storePass);
    }
     ...
     * @param content path.
     * @return a new {@link WebSocket} instance, which allows to securely connect with the server. */
    public WebSocket newSecureWebSocket(final String host, final int port, final Files gdxFiles, final String keystorePath, final String storeType, final String keyPass, final String storePass, final String content) {
        return WebSockets.newSecureSocket(WebSockets.toSecureWebSocketUrl(host, port, content), gdxFiles, keystorePath, storeType, keyPass, storePass);
    }
}
